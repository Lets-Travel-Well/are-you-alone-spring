package com.ssafy.rualone.global.util;
...
public class MakeMapUtils {
    ...
    public static List<JourneyPlacePathDto> makeShortestPath(List<JourneyPlacePathDto> journeyPlacePathDto){
        List<JourneyPlacePathDto> journeyPlacePathResponse = new ArrayList<>();
        size = journeyPlacePathDto.size();
        ...
        for(int i = 0; i < size; i++){
            ...
            for(int j = i; j < size; j++){
                double len = getLength(journeyPlacePathDto.get(i), journeyPlacePathDto.get(j));
                map[i][j] = len;
                map[j][i] = len;
            }
        }
        ...
        for(Integer temp : list){
            journeyPlacePathResponse.add(journeyPlacePathDto.get(temp));
        }
...
//            answer = Math.min(answer, compare);
    ...
    private static double getLength(JourneyPlacePathDto myAttractionDto1, JourneyPlacePathDto myAttractionDto2){
